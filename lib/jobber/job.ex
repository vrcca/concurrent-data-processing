defmodule Jobber.Job do
  ...
  def init(args) do
    ...
    Logger.info("Job started #{state.id}")
  ...
  def handle_continue(:run, state) do
    ...
    if new_state.status == "errored" do
      Process.send_after(self(), :retry, _in_5s = 5_000)
      {:noreply, new_state}
    else
  ...
  defp handle_job_result(:error, state = %{status: "new"}) do
    Logger.warn("Job errored #{state.id}")
    %Jobber.Job{state | status: "errored"}
  end

  defp handle_job_result(:error, state = %{status: "errored"}) do
